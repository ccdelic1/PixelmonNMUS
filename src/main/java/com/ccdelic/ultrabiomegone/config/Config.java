...
public final class Config {
    ...
    public static final ModConfigSpec.DoubleValue RELOCATED_ULTRA_BEAST_ENCOUNTER_CHANCE;
    ...
    static {
        ...
        RELOCATION_RARITY_DIVISOR = b
            .comment("Divide the relocated spawns' rarity by this (default 5.0 -> UB 0.5 becomes 0.1).")
            .defineInRange("relocationRarityDivisor", 5.0D, 0.0001D, 1000.0D);
        ...
        RELOCATED_ULTRA_BEAST_ENCOUNTER_CHANCE = b
            .comment("Independent cap (0..1) on how often a relocated Ultra Beast is even considered a spawn",
                     "candidate at all, applied via Pixelmon's own SpawnInfo.chance pre-filter -- BEFORE rarity ever",
                     "comes into play. This is what actually fixes 'UBs are common in a sparse biome pool': Pixelmon's",
                     "weighted spawn picker (SelectionAlgorithm.choose) special-cases a suitable-spawn list of exactly",
                     "one candidate and returns it unconditionally, ignoring its rarity value entirely. A relocated UB",
                     "sitting in a thin overworld pool (e.g. vanilla's single desert biome, or a low-level player whose",
                     "party level filters out most of the biome's normal spawns) can easily end up as that lone",
                     "candidate, which would make relocationRarityDivisor irrelevant and the UB spawn essentially",
                     "every time the biome's spawner fires. This value bounds that independently of pool size/",
                     "composition: e.g. 0.12 means the UB is only even in the running 12% of the time a spawn is",
                     "attempted there, no matter how many (or how few) other Pokemon could otherwise compete for the",
                     "spot. Set to 1.0 to disable this cap and rely on relocationRarityDivisor alone.")
            .defineInRange("relocatedUltraBeastEncounterChance", 0.12D, 0.0001D, 1.0D);
        b.pop();
