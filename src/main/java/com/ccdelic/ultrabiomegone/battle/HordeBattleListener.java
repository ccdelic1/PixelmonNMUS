...
public final class HordeBattleListener {
    ...
    public static void onBattleStart(BattleStartedEvent.Pre event) {
        UltraBiomeGone.debugLog("HordeBattleListener: BattleStartedEvent.Pre fired");
        if (!Config.HORDE_ENABLED.get()) {
            UltraBiomeGone.debugLog("HordeBattleListener: horde battles disabled in config, skipping");
            return;
        }
        BattleParticipant player = findPlayer(event);
        if (player == null || player.getPlayer() == null) {
            UltraBiomeGone.debugLog("HordeBattleListener: no player participant found, skipping");
            return;
        }
        // Gate 1: overworld only.
        if (player.getPlayer().level().dimension() != Level.OVERWORLD) {
            UltraBiomeGone.debugLog("HordeBattleListener: not in overworld, skipping");
            return;
        }
        // Gate 2: a plain 1v1.
        if (event.getTeamOne().length > 1 || event.getTeamTwo().length > 1) {
            UltraBiomeGone.debugLog("HordeBattleListener: not a 1v1 battle (team1={}, team2={}), skipping", event.getTeamOne().length, event.getTeamTwo().length);
            return;
        }
        ...
        if (wildPokemon == null || !(wildPokemon.getEntity() instanceof PixelmonEntity entity)) {
            UltraBiomeGone.debugLog("HordeBattleListener: no wild Pixelmon participant, skipping");
            return;
        }
        // Gate 3: the initiating wild Pokémon is AGGRESSIVE.
        if (entity.getAggression() != Aggression.AGGRESSIVE) {
            UltraBiomeGone.debugLog("HordeBattleListener: wild Pokémon is not AGGRESSIVE ({}), skipping", entity.getAggression());
            return;
        }
        // Gate 4: configurable roll (default 2%) and there aren't already extra participants.
        int participants = event.getBattleController().participants.size();
        boolean chanceHit = RandomHelper.getRandomChance(Config.HORDE_CHANCE.get());
        if (participants > 2 || !chanceHit) {
            UltraBiomeGone.debugLog("HordeBattleListener: participants={} > 2 or chance roll failed ({}), skipping", participants, chanceHit);
            return;
        }
        if (PixelmonSpawning.coordinator == null) {
            UltraBiomeGone.debugLog("HordeBattleListener: PixelmonSpawning.coordinator is null, skipping");
            return;
        }
        ...
        if (!(spawner instanceof PlayerTrackingSpawner pSpawner)) {
            UltraBiomeGone.debugLog("HordeBattleListener: spawner is not PlayerTrackingSpawner, skipping");
            return;
        }
        ...
        UltraBiomeGone.debugLog("HordeBattleListener: horde triggered! Cancelling 1v1 and spawning {} extra Pokémon for player {}", Config.HORDE_MAX_EXTRA.get(), player.getName().getString());
        ...
        pSpawner.getTrackedBlockCollection(player.getPlayer(), 0.0F, 0.0F,
            ...
            .exceptionally(ex -> {
                if (ex instanceof java.util.concurrent.TimeoutException) {
                    UltraBiomeGone.debugLog("HordeBattleListener: spawn timed out after 15s");
                } else {
                    UltraBiomeGone.LOGGER.warn("[PixelmonNMUS] Horde battle spawn failed", ex);
                    UltraBiomeGone.debugLog("HordeBattleListener: spawn failed with exception: {}", ex.getMessage());
                }
                return null;
            });
    }
    ...
    public static void onBattleEnd(BattleEndEvent event) {
        ...
        for (BattleParticipant participant : event.getBattleController().participants) {
            if (participant.isWild() && participant.getEntity() != null
                ...
                UltraBiomeGone.debugLog("HordeBattleListener: removing temporary horde Pokémon {} after battle end", participant.getEntity().getName().getString());
                participant.getEntity().remove(RemovalReason.DISCARDED);
                removed++;
            }
        }
        if (removed > 0) {
            UltraBiomeGone.debugLog("HordeBattleListener: cleaned up {} horde Pokémon after battle", removed);
        }
    }
